...
namespace eosiosystem {
   ...
   void native::setabi( const name& acnt, const std::vector<char>& abi ) {
      ...
      check (system_contract::checkPermission(acnt, "setcontract")==1, "You are not authorised to setabi");  // XEC Check Permissions
   ...
   void native::setcode( const name& account, uint8_t vmtype, uint8_t vmversion, const std::vector<char>& code ) {
      check (system_contract::checkPermission(account, "setcontract")==1, "You are not authorised to setcode");  // XEC Check Permissions
   }
  ...
  /*-------------------------------------*/
   ...
   uint8_t system_contract::checkPermission(name acc, std::string permission){
      const std::map<std::string,uint8_t> accperm = eosio::eosioxec::get_priv ( system_contract::xec_account, acc);
